 ...
 * 
 * 1. After flash this code to the XMC4700 Relax Kit, the angle alignment will be applied, so the drive
 * needs to be detached from the BLDC Motor. 
 * !Gripper must be opened to their fullest extent to release the gear.
 * 
 * 2. After the angle alignment, attach the gears and you can start controlling the gripper clip opening 
 * and closing. Pressing button 1 on the board will start to close the gripper and pressing button 2 will 
 * open the gripper. 
 * !There is no upper limit set for opening, so it is possible that the gears may detach after exceeding the maximum.
 *                              
 * 3.  Open the serial monitor and you can see the data of the magnetic sensors placed under the tpu material 
 * on the top of the clip. When the gripping clip actually grabs the object and generates pressure, the data changes.   
 * 
 * This is an basic example, you can be creative to make this gripper better!
 *  
 */
#include <SimpleFOC.h>
#include "config.h"
...
// BLDC motor instance BLDCMotor (polepairs, motor phase resistance, motor KV rating, motor phase inductance)
BLDCMotor motor = BLDCMotor(7, 0.24, 360, 0.000133); // 7 pole pairs, 0.24 Ohm phase resistance, 360 KV and 0.000133H
...
#if ENABLE_MAGNETIC_SENSOR
/**
 * @brief Calibrates the magnetic field sensor by calculating the average offsets
 *        for the X, Y, and Z axes over a series of samples.
 */
